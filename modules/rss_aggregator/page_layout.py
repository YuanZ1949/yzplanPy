...
class _RssPageWidget(_RssPageWidget):  # type: ignore[reportGeneralTypeIssues]
    ...
    def _build_three_col(self, rss_c, root):
        ...
        _side_col.setStyleSheet(
            f"QFrame#rssSideCol {{ background: {rss_c['rss_panel']}; border: 1px solid {rss_c['rss_border']}; "
            f"border-radius: {rss_c['rss_radius_2xl']}px; padding: {rss_c['rss_card_padding']}; }}")
        ...
        _list_col.setStyleSheet(
            f"QFrame#rssListCol {{ background: {rss_c['rss_panel']}; border: 1px solid {rss_c['rss_border']}; "
            f"border-radius: {rss_c['rss_radius_2xl']}px; padding: {rss_c['rss_card_padding']}; }}")
        ...
        self.item_list.setStyleSheet(
            ("QListWidget {{ background: transparent; border: none; }}"
             ...
             "border: 1px solid {rss_accent}; }}").format(**rss_c)
        )
        ...
        self.lb_page.setStyleSheet(f"color: {_pg_c['rss_text_secondary']}; padding: 0 4px;")
        ...
        self.lb_total.setStyleSheet(f"color: {_pg_c['rss_text_secondary']}; padding-right: 6px;")
        ...
        _preview_col.setStyleSheet(
            f"QFrame#rssPreviewCol {{ background: {rss_c['rss_panel']}; border: 1px solid {rss_c['rss_border']}; "
            f"border-radius: {rss_c['rss_radius_2xl']}px; padding: {rss_c['rss_card_padding']}; }}")
        ...
        self._summary_title.setStyleSheet(
            f"QLabel {{ font-size: {rss_c['rss_font_lg']}px; font-weight: 700; background: transparent; color: {rss_c['rss_text']}; }}"
        )

        self._summary_status = QtWidgets.QLabel("")
        self._summary_status.setFixedHeight(sizing()["rss_summary_status_height"])
        self._summary_status.setStyleSheet(
            f"QLabel {{ font-size: {sizing()['rss_font_sm']}px; padding: {sizing()['rss_pill_padding']}; "
            f"border-radius: {sizing()['rss_radius_3xl']}px; font-weight: 600; }}"
        )

        self._summary_meta = QtWidgets.QLabel("")
        self._summary_meta.setWordWrap(True)
        self._summary_meta.setStyleSheet(
            f"QLabel {{ color: {rss_c['rss_text_secondary']}; font-size: {rss_c['rss_font_md']}px; "
        ...
        self._summary_desc.setStyleSheet(
            f"QLabel {{ font-size: 12.5px; background: transparent; color: {rss_c['rss_text']}; line-height: 1.7; }}"
        )
        ...
        _sep_line.setFixedHeight(sizing()["rss_sep_height"])
        _sep_line.setStyleSheet(f"background: {rss_c['rss_border']};")
        ...
        self._preview_placeholder.setStyleSheet(
            f"color: {rss_c['rss_text_faint']}; font-size: {rss_c['font_size_md']}px; "
            f"padding: {rss_c['rss_empty_padding']};")
        self._preview_stack = QtWidgets.QStackedWidget()
        self._preview_stack.setStyleSheet(
            ("QStackedWidget {{ background: transparent; border-radius: {rss_radius_md}px; }}").format(**rss_c)
        )
